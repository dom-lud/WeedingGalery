...
6. **Test Design Brief i macierz ryzyk**: PRZED napisaniem testów zapisz wymagania i ryzyka, błędną implementację, którą test ma wykryć, oraz scenariusze pozytywne, negatywne i graniczne. Każde ryzyko przypisz do najniższej wiarygodnej warstwy testu. Dla limitu sprawdź `N-1`, `N`, `N+1`; dla idempotency replay i konflikt; dla ownership wariant dozwolony i zabroniony; dla storage także kompensację i brak osieroconych danych.
7. **Coverage nie zastępuje scenariuszy**: Progi coverage są wyłącznie zapadką regresyjną. Nie wolno obniżać progów ani dopisywać testów bez istotnych asercji tylko po to, aby CI przeszło. Zakończenie zadania wymaga zarówno zielonych progów, jak i kompletnej macierzy ryzyk.
8. **Realne zależności i accessibility**: Migracje oraz zachowania zależne od silnika danych testuj na rzeczywistej wspieranej bazie, jeśli różnią się od H2. Krytyczne ekrany UI muszą przejść automatyczny audit WCAG A/AA oraz test klawiatury, focusu i responsywności.
